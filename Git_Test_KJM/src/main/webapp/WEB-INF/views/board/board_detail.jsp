...
<!-- 								3) substringAfter() 함수 이용하여 지정된 문자 다음부터 끝까지 추출 -->
<!-- 									substring() : ${fn:substringAfter(board.board_file1, '_')} -->
<%-- 							<a href="${pageContext.request.contextPath}/resources/upload/${board.board_file1 }" download="${fn:substringAfter(board.board_file1, '_')}"> --%>
<%-- 								${board.board_file1 }<input type="button" value="다운로드"> --%>
<!-- 							</a> -->
<!-- 						</div> -->
...
<%-- 						<div>${board.board_file3 }</div> --%>
						...
						<!-- List 객체("fileList") 크기만큼 일반반복문을 통해 파일명 출력 -->
<%-- 						<c:forEach var="i" begin="0" end="${fn:length(fileList) - 1}"> fn사용하지 않으려면 아래처름 size 활용하면 됨. --%>
<%-- 						<c:forEach var="i" begin="0" end="${fileList.size() - 1}"> --%>
<%-- 							${fileList[i]} : ${originalFileList[i]}  --%>
<%-- 						</c:forEach> --%>

						<!-- List 객체("fileList") 크기만큼 향상된 반복문을 통해 파일명 출력 -->
						<!--  varStatus : 사용하면 됨???????????????? 그러면 일반for문 처럼 반복가능함. -->
						<!--  기본 반복 객체 지정 후 다른 객체는 varStatus 속성을 통해 index 값 사용가능 -->
						<c:forEach var="file" items="${fileList}" varStatus="status">
							<!--  단, 파일명이 존재할 경우에만 출력 -->
							<c:if test="${not empty file}">
								<div>
<%-- 									${file} : ${originalFileList[status.index]} --%>
									
									<a href="${pageContext.request.contextPath}/resources/upload/${file}" download="${originalFileList[status.index]}">
										${file}<input type="button" value="다운로드">
		 							</a> 
		...
		<section id="commandCell">
			...
			<!-- 목록버튼은 항상 표시하고, 이전 페이지로 돌아가기로 처리 -->
