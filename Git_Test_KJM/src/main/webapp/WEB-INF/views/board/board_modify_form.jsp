...
<!-- 					파일명 존재 여부 판별하여 업로드 버튼 or 파일명과 삭제버튼 표시 -->
					...
					<!-- 
						반복요소 인덱스값 또는 순서번호 확인을 ㅜ위해 varStatus 속성 사용
						반복 인덱스 : 속성명.index 반복순서번호 : 속성명.count
					 -->
					<c:forEach var="file" items="${fileList}" varStatus="status">
						<div class="board_file">
							<!-- 파일명이 존재할 경우 원본 파일명 표시하고
								 아니면 파일 업로드 버튼 표시
							 -->
							 <c:choose>
							 	<c:when test="${not empty file}">
							 		...
							 		<!-- 파일다운로드링크(download-icon.png 이밎)생성 -->
							 		<a href="${pageContext.request.contextPath}/resources/upload/${file}" download="${originalFileList[status.index]}">
										<img class="img_btn" alt="dowmload" src="${pageContext.request.contextPath}/resources/images/download-icon.png"></a> 
		 							<!-- 파일삭제 링크(delete-icon.png 이미지) 생성(개별 삭제 용도) -->
		 							
		 							<!-- 하이퍼링크 클릭시 deleteFile() 함수 호출(파라미터: 글번호, 실제 파일명, 카운트번호) -->
							 		<a href="javascript:deleteFile(${board.board_num}, '${file}', ${status.count})">
										<img class="img_btn" alt="dowmload" src="${pageContext.request.contextPath}/resources/images/delete-icon.png">
		 							</a> 
		 							<!-- 
		 								만약 파일명만 표시할 때 파일 업로드 요소도 파라미터로 전달하려면 (파라미터값은 널스트링)
		 								파일업로드 요소 추가 후 hidden속성으로 숨김처리가능 또는 input type="hidden" 사용) - hidden 안하면 파일선택 버튼이 넘어간다. 또는 input type="hidden"ㅇㅡ로 처리해도 된다. 
		 							 -->
