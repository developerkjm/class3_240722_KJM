...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %> <!-- prefix는 자유롭게 써. -->
			...
			<table border="1">
				...
				<tr>
					...
					<td>
						...
						<input type="text" name="id" id="id" placeholder="4 ~ 8글자 사이 입력" onblur="checkIdLength()" value="${member.id}" readonly="readonly">
				...
				<tr>
					<!-- 새 비밀번호와 새 비밀번호 확인은 비밀번호 변경시에만 입력하도록 필수항목에서 제외 				 -->
					<th>새 비밀번호</th>
					<td>
				...
				<tr>
					<th>E-Mail</th>
					<td>
					<!--  이메일주소 분리 (기준문자열 : @) 후 각각 표시
						  JSTP - functions 라이브러리의 split() 함수 필요
...
<!-- 					 	분리된 각 배열의 문자열을 출력 -->
				...
				<tr>
					...
					<td>
						<select name="job">
							<!-- member.job 값에 따라 직업 항목 셀렉트박스의 option 태그항목선택(selected) -->
							<!-- c:if 태그 사용하여 직업이 XXX일 경우 selected 속성 추가 -->
							<!-- when otherwise 보다 이 방법이 코드가 짧다								 -->
				...
				<tr>
					<th>취미</th>
					<td>
<!-- member.hobby 값에 해당 취미 포함 여부를 판별하여 해당 항목 체그(checked) -->
<!-- split() 함수를 통해 분리 후 판별하거나 contains() 함수를 통해 분리없이 문자열 포함여부 검색  -->
<%-- => ${fn:contains(원본문자열, 탐색할문자열)}					 --%>
				...
				<tr>
					<th>가입동기</th>
					<td>
						<textarea rows="5" cols="40" id="motivation" name="motivation">
							${member.motivation}
						</textarea>
					</td>
				</tr>
